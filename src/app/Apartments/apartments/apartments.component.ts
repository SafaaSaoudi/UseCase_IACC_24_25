...
export class ApartmentsComponent {
  constructor( private cm:CommonService, private apartS: ApartmentService) { }
  ...
  ngOnInit(){
   // console.log(this.cm.getSameValueOf(this.listApartments,"surface",150));
    this.apartS.getApartments().subscribe((data)=>{
      this.listApartments = data;
    } );
  }

  delete(id:number){
    this.apartS.deleteApartment(id).subscribe(()=>{
      this.listApartments = this.listApartments.filter(apart=> apart.id != id);
    } );
  }
}
